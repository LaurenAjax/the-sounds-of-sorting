...
public class Sorts {
    ...
    public static <T extends Comparable<? super T>> List<SortEvent<Integer>> bubbleSort(T[] arr) {
        ...
        for (int i = 1; i < arr.length; i++) {
            for (int j = 0; j < arr.length - i; j++) {
                list.add(new CompareEvent(arr, j, j + 1));
    ...
    public static <T extends Comparable<? super T>> List<SortEvent<Integer>> selectionSort(T[] arr) {
        ...
        for (int i = 0; i < arr.length - 1; i++) {
            ...
            for (int j = i + 1; j < arr.length; j++) {
                list.add(new CompareEvent(arr, min, j));
    ...
    public static <T extends Comparable<? super T>> List<SortEvent<Integer>> insertionSort(T[] arr) {
        ...
        for (int i = 1; i < arr.length; i++) {
            for (int j = i - 1; j >= 0; j--) {
                list.add(new CompareEvent(arr, j + 1, j));
     ...
     * @param list
     */
    public static <T extends Comparable<? super T>> void merge(T[] arr, T[] leftArr, T[] rightArr, int left, int right, List<SortEvent<Integer>> list) {
        ...
        while (leftInd < left && rightInd < right) {
            list.add(new CompareEvent(leftArr, leftInd, rightInd));
     ...
     * @param list
     */
    public static <T extends Comparable<? super T>> void sort(T arr[], int length, List<SortEvent<Integer>> list) {
        if (length < 2) {
            return;
        }
        int mid = length / 2;
     ...
     * @param list
     */
    public static <T extends Comparable<? super T>> void quick(T[] arr, int lo, int hi, List<SortEvent<Integer>> list) {
        if (lo >= hi) {
            return;
        }
        int pivot = hi;
        ...
        while (lowerPtr < higherPtr) {
            list.add(new CompareEvent(arr, lowerPtr, pivot));
            while (arr[lowerPtr].compareTo(arr[pivot]) <= 0 && lowerPtr < hi) {
                lowerPtr++;
            }
            list.add(new CompareEvent(arr, higherPtr, pivot));
    ...
    public static <T extends Comparable<? super T>> List<SortEvent<Integer>> mySort(T[] arr) {
        ...
        int min = 0;
        for (int i = 1; i < arr.length; i++) {
            list.add(new CompareEvent(arr, i, min));
        ...
        for (int j = 1; j < arr.length; j++) {
            for (int k = 0; k < arr.length; k++) {
                list.add(new CompareEvent(arr, k, min));
                list.add(new CompareEvent(arr, k, j - 1));
                if (arr[k].compareTo(arr[min]) < 0 && arr[k].compareTo(newArr[j - 1]) >= 0) {
                    min = k;
                }
            }
            newArr[j] = arr[min];
        ...
        for (int l = 0; l < arr.length; l++) {
            list.add(new CopyEvent(newArr, l, l));
            arr[l] = newArr[l];
        }
        return list;
    }
